...
exports.GET_TWEET_DETAILS = async (
  message,
  HANDLE_SEND_MESSAGE,
  HANDLE_EDIT_MESSAGE,
  response,
  ...
  try {
    ...
    if (
      ...
      const media_files =
        data.extended_entities.media[0].video_info.variants.filter(
          (file) => file.content_type == 'video/mp4'
        );
      const greatest_bitrate = media_files.sort(
        (fileA, fileB) => fileB.bitrate - fileA.bitrate
      )[0];
...
exports.HANDLE_GET_BOT_MENTIONS = async (last_tweet_id, HANDLE_SEND_VIDEO) => {
  ...
  try {
    ...
    await data.forEach(async (tweet) => {
      ...
      const HaveIReplied = BotTimeline.find(
        (t) => t.in_reply_to_status_id_str === tweet.id_str
      );
    ...
    const refrenced_tweets = await HANDLE_GET_REFERENCED_TWEET(
      refrenced_tweet_ids
    );
    //Work on the tweet
    HANDLE_EXTRACT_VIDEO_FROM_TWEET(
