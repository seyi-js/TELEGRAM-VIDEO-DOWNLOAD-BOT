...
const { LOOK_UP_USER_BY_SCREEN_NAME, HANDLE_SEND_DIRECT_MESSAGE } =
  require('../twitter/utils')();
const { screen_name, VERIFICATION_ATTEMPT } = require('../../misc/utils');
const { splitVideo } = require('./utils');
const fs = require('fs');
  ...
  const HANDLE_RESPOND_TO_MESSAGE = async (message) => {
    ...
    if (user && user.isBlocked) {
      return HANDLE_SEND_MESSAGE(
        message.chat.id,
        `You are currently restricted from using this Bot. Kindly reach out the admin.`,
    ...
    switch (message.text) {
      ...
      case '/start':
        ...
        return HANDLE_SEND_MESSAGE(message.chat.id, message_to_send, {
          reply_to_message_id: message.message_id,
        });
      ...
      default:
        return HANDLE_SEND_MESSAGE(
          ...
          `Hi @${
            message.from.first_name || message.from.username
          },\n\nI do not understand the command you entered.`,
          { reply_to_message_id: message.message_id }
        );
    }
  };
  ...
  const HANDLE_URLS = async (message) => {
    ...
    if (match) {
      switch (match.origin) {
        ...
        case 'https://x.com':
          const response = await HANDLE_SEND_MESSAGE(
            message.chat.id,
            'Generating your video... ⏳'.italics(),
        ...
        default:
          return HANDLE_SEND_MESSAGE(
            ...
            `Hi @${
              message.from.first_name || message.from.username
            },\nYou have entered an invalid or unsupported URL.`,
            { reply_to_message_id: message.message_id }
          );
      }
    } else {
  ...
  const MANAGE_REGISTRATION = async (message) => {
    try {
      ...
      switch (social[0].toUpperCase()) {
        case 'TWITTER':
          if (!social[1]) {
            return await HANDLE_SEND_MESSAGE(
              message.chat.id,
              ` Kindly check if the info was entered in the correct order.\ne.g TWITTER - YOUR_SCREEN_NAME`,
              { reply_to_message_id: message.message_id, parse_mode: 'html' }
            );
          }
          ...
          if (!user) {
            return HANDLE_SEND_MESSAGE(
              message.chat.id,
              `The user with this screen name @${social[1]} could not be found. Kindly check if the screen name was entered in the correct order.`,
              { reply_to_message_id: message.message_id }
            );
          }
          ...
          if (!DBuser) {
            return HANDLE_SEND_MESSAGE(
              message.chat.id,
              `Kindly restart this Bot by clearing the application history.`,
              { reply_to_message_id: message.message_id }
            );
          }
          ...
          if (DBuser.twitter_info.info_verified) {
            return HANDLE_SEND_MESSAGE(
              message.chat.id,
              `Your info has been registered already.`,
              { reply_to_message_id: message.message_id }
            );
          }
          ...
          return HANDLE_SEND_MESSAGE(
            message.chat.id,
            `An OTP has been sent to your Twitter inbox. Kindly copy it and enter it in this format.\n\n TWITTEROTP - YOUR OTP\n  e.g\nTWITTEROTP - 123456`,
            { reply_to_message_id: message.message_id }
          );
        ...
        case 'TWITTEROTP':
          if (!social[1]) {
            return await HANDLE_SEND_MESSAGE(
              message.chat.id,
              ` Kindly check if the info was entered in the correct order.\ne.g TWITTEROTP - YOUR_OTP`,
              { reply_to_message_id: message.message_id, parse_mode: 'html' }
            );
          }
          const opt_user = await User.findOne({
            telegram_user_id: message.from.id,
          });

          await HANDLE_SEND_MESSAGE(
            message.chat.id,
            'Verification in progress... ⏳'.italics(),
            { reply_to_message_id: message.message_id, parse_mode: 'html' }
          );
          ...
          if (!opt_user) {
            return HANDLE_SEND_MESSAGE(
              message.chat.id,
              `Kindly restart this Bot by clearing the chat history.`,
              { reply_to_message_id: message.message_id }
            );
          }
          ...
          if (parseInt(opt_user.twitter_info.otp) !== parseInt(social[1])) {
            opt_user.twitter_info.verification_attempt =
              opt_user.twitter_info.verification_attempt + 1;
            opt_user.twitter_info.verification_attempt === VERIFICATION_ATTEMPT
              ? (opt_user.isBlocked = true)
              : null;
            await opt_user.save();
            return HANDLE_SEND_MESSAGE(
              message.chat.id,
              `You have entered an incorrect OTP. You have ${
                parseInt(VERIFICATION_ATTEMPT) -
                parseInt(opt_user.twitter_info.verification_attempt)
              } attempts left.`,
              { reply_to_message_id: message.message_id }
            );
          }
          ...
          return HANDLE_SEND_MESSAGE(
            message.chat.id,
            `Hurray!!!, your twitter account has been linked successfully. You can now mention me under any tweet containing a video and I will send it right into your telegram inbox.`,
            { reply_to_message_id: message.message_id }
          );
      ...
      return HANDLE_SEND_MESSAGE(
        ...
        `Hi @${
          message.from.first_name || message.from.username
        },\nYour twitter account cannot be linked at this moment. Please try again later.`,
        { reply_to_message_id: message.message_id }
      );
    }
  };

  const HANDLE_SAVE_VIDEO = async (message) => {
    await HANDLE_SEND_MESSAGE(
      message.chat.id,
      `Processing video... ⏳`.italics(),
      { reply_to_message_id: message.message_id, parse_mode: 'html' }
    );

    const video_duration = message.video.duration;

    let vid = await Bot.downloadFile(message.video.file_id, './videos');

    //split video into 30 seconds each

    const splitted_filenames = [];

    for (let i = 0; i < video_duration / 30; i++) {
      let time = i * 30;
      const file_name = `videos${i}.mp4`;
      await splitVideo(vid, time, 30, file_name);
      splitted_filenames.push(file_name);
    }

    //upload video to telegram
    for (let i = 0; i < splitted_filenames.length; i++) {
      await Bot.sendVideo(
        message.chat.id,
        fs.createReadStream(splitted_filenames[i]),
        {
          reply_to_message_id: message.message_id,
          caption: `Video ${i + 1} of ${splitted_filenames.length}`,
        }
      );
    }
    ...
    //delete splitted videos from local storage
    for (let i = 0; i < splitted_filenames.length; i++) {
      fs.unlinkSync(splitted_filenames[i]);
    }
  };
